...
- **FastWMS (Warehouse Management Module)**: Integrated WMS module for location-level inventory tracking within the CD. Models: `WmsWarehouse` (id, code unique, name), `WmsLocation` (id, warehouse_id FK, location_code, location_type, max_pallets, is_active; unique constraint on warehouse_id+location_code), `WmsInventory` (id, warehouse_id FK, location_id FK, product_id FK, on_hand_units, allocated_units, on_hand_pallets; unique constraint on warehouse_id+location_id+product_id), `WmsMovement` (id, movement_type, product_id FK, from_location_id, to_location_id, units, pallets, note, created_at, created_by_user_id). Default records created on boot: warehouse `MAIN` and location `BULK-DEFAULT` (type BULK). Stock CD upload accepts optional WMS columns: `warehouse`, `location_code`, `location_type`, `pallets`, `max_pallets`, `note`. Also accepts `units` as alias for `quantity`. Validation rejects negative units/pallets; warns (but allows) when pallets > max_pallets. Upload modes: `append` adds to WmsInventory + creates WmsMovement CD_RECEIPT records; `replace_today` clears StockCD for date, WMS stays additive; `replace_all` clears WmsInventory + WmsMovement + StockCD then loads fresh. After WMS load, StockCD is synced as compatibility summary (aggregate by SKU). Helper `get_cd_total_stock_units()` returns SUM(WmsInventory.on_hand_units) when WMS data exists, else falls back to StockCD. Indexes on WmsInventory(product_id), WmsInventory(location_id), WmsLocation(location_code). Sidebar group "FastWMS" with "Inventory by Location" page (`/wms/inventory`): read-only table view joining WmsInventory+WmsLocation+WmsWarehouse+Product with KPI row (locations, SKUs, total units), location type badges (PICK=blue, BULK=gray), red text for negative available units. Permission: `wms:view` (Admin, Management, WarehouseOps).
