...
- **FastWMS Pick Waves**: Auto-created when a distribution plan is approved and sent to FastPlanner. Models: `WmsPickWave` (header with plan_id, status, priority, urgency_level, priority_rank, assigned_to, timing fields, denormalized counters), `WmsPickTask` (one per consolidated SKU, with product_name, category, short_qty, location_hint), `WmsPickReservation` (per-location reservation breakdown), `WmsOperatorKpiDaily` (daily operator performance rollup). `wms_create_pick_wave_for_plan()` consolidates demand by SKU, reserves inventory from highest-available locations, sets wave READY_TO_ASSIGN or NEEDS_REVIEW. Workflow: READY_TO_ASSIGN → ASSIGNED → IN_PROGRESS → PICKED → CLOSED (+ NEEDS_REVIEW). Priority gating: URGENT waves block non-urgent assignment/start. Finalize decrements `on_hand_units` and `allocated_units` using reservation records. FastPlanner kanban cards show WMS status badge + "Ver Picking" button. Routes: `/wms/waves` (list+filter+assign modal), `/wms/my-waves` (operator queue), `/wms/waves/<id>` (detail with KPIs, task table, collapsible reservations), `/wms/waves/<id>/pick` (line-by-line picking with PICKED/SHORT per task), `/wms/kpis` (operator performance ranking). Sidebar: "WMS Waves", "Mi Picking", "WMS KPIs".
